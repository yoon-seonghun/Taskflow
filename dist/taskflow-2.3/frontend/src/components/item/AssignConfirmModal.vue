  ...
  <Modal :model-value="show" title="업무 배정" size="md" @close="emit('close')">
    <div class="p-4 space-y-5">
      ...
      <div class="text-center">
        <div class="inline-flex items-center gap-2 px-3 py-2 bg-gray-100 dark:bg-gray-800 rounded-lg">
          <div class="w-8 h-8 rounded-full bg-primary-100 dark:bg-primary-900 flex items-center justify-center">
            <svg class="w-4 h-4 text-primary-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
            </svg>
          </div>
          <span class="font-medium text-gray-900 dark:text-white">{{ assigneeName }}</span>
        </div>
        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">님에게 업무를 배정하시겠습니까?</p>
      </div>

      <!-- 업무 정보 -->
      <div class="px-3 py-2 bg-gray-50 dark:bg-gray-800 rounded-lg">
        <div class="text-xs text-gray-500 dark:text-gray-400 mb-1">업무</div>
        <div class="text-sm font-medium text-gray-900 dark:text-white truncate">{{ itemTitle }}</div>
      </div>

      <!-- 권한 수준 선택 -->
      <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">권한 수준</label>
        <div class="space-y-2">
          <label
            v-for="option in permissionOptions"
            :key="option.value"
            class="flex items-start gap-3 p-3 border dark:border-gray-700 rounded-lg cursor-pointer transition-colors"
            :class="[
          ...
          >
            ...
            <div>
              <div class="font-medium text-sm text-gray-900 dark:text-white">
                {{ option.label }}
                <span v-if="option.value === 'EDIT'" class="ml-1 text-xs text-primary-600 dark:text-primary-400">(권장)</span>
              </div>
      ...
      <div class="space-y-3">
        <label class="flex items-center gap-2 cursor-pointer">
          ...
          />
          <span class="text-sm text-gray-700 dark:text-gray-300">이메일로 알림 발송</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer">
          <input
            type="checkbox"
            ...
            class="rounded text-primary-600 focus:ring-primary-500"
          />
          <span class="text-sm text-gray-700 dark:text-gray-300">앱 내 알림 발송</span>
        </label>
      </div>

      <!-- 에러 메시지 -->
      <div v-if="errorMessage" class="px-3 py-2 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg">
