...
<template>
  <div class="subtask-list">
    ...
    <div class="flex items-center gap-2 py-1">
      ...
      <button
        v-if="hasChildren"
        class="flex items-center gap-1 text-[12px] text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200"
        @click="toggleExpand"
      >
        <svg
        ...
        <span>하위 업무</span>
        <span v-if="childCountText" class="text-gray-400 dark:text-gray-500">({{ childCountText }})</span>
      </button>

      <!-- 진행률 바 -->
      <div v-if="hasChildren && (parentItem.childCount ?? 0) > 0" class="flex-1 max-w-[100px] h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
        <div
      ...
      <button
        ...
        class="flex items-center gap-1 px-2 py-0.5 text-[11px] text-gray-500 hover:text-primary-600 hover:bg-primary-50 dark:text-gray-400 dark:hover:text-primary-400 dark:hover:bg-primary-900/50 rounded transition-colors"
    ...
    <div v-if="showAddForm" class="flex items-center gap-2 py-1 pl-5">
      <input
        ...
        type="text"
        class="flex-1 h-7 px-2 text-[13px] border border-gray-300 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-200 rounded focus:outline-none focus:ring-1 focus:ring-primary-500 focus:border-primary-500"
      ...
      <button
        class="px-2 py-1 text-[12px] text-gray-600 hover:text-gray-800 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-gray-200 dark:hover:bg-gray-700 rounded transition-colors"
        @click="cancelAdd"
      >
        취소
      </button>
    ...
    <div v-if="isExpanded" class="pl-5 space-y-0.5">
      <!-- 로딩 -->
      <div v-if="loading" class="py-2 text-[12px] text-gray-400 dark:text-gray-500">
        로딩 중...
      </div>
      ...
      <template v-else>
        <div
          ...
          :key="child.itemId"
          class="group flex items-center gap-2 py-1 px-2 hover:bg-gray-50 dark:hover:bg-gray-800 rounded cursor-pointer"
          @click="selectChild(child)"
        >
          <!-- 완료 체크박스 -->
          <button
            class="flex-shrink-0 w-4 h-4 border rounded transition-colors"
            :class="child.status === 'COMPLETED' ? 'bg-green-500 border-green-500 text-white' : 'border-gray-300 hover:border-green-500 dark:border-gray-600 dark:hover:border-green-400'"
          ...
          <span
            ...
            :class="child.status === 'COMPLETED' ? 'line-through text-gray-400 dark:text-gray-500' : 'text-gray-700 dark:text-gray-200'"
          >
            {{ child.title }}
          </span>
          ...
          <!-- 하위 업무 카운트 -->
          <span v-if="(child.childCount ?? 0) > 0" class="flex-shrink-0 text-[11px] text-gray-400 dark:text-gray-500">
            {{ child.completedChildCount ?? 0 }}/{{ child.childCount }}
          </span>

          <!-- 삭제 버튼 (hover 시) -->
          <button
            class="flex-shrink-0 opacity-0 group-hover:opacity-100 p-1 text-gray-400 hover:text-red-500 dark:text-gray-500 dark:hover:text-red-400 transition-all"
            title="삭제"
        ...
        <!-- 빈 상태 -->
        <div v-if="children.length === 0 && !loading" class="py-2 text-[12px] text-gray-400 dark:text-gray-500 text-center">
          하위 업무가 없습니다
        </div>
      </template>
    </div>
