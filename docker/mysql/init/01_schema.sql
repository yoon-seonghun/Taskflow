...
-- ============================================
ALTER TABLE TB_ITEM
    ADD COLUMN TRANSFERRED_FROM BIGINT NULL COMMENT '이관 원본 보드 ID' AFTER DELETED_BY,
    ADD COLUMN TRANSFERRED_AT DATETIME NULL COMMENT '이관 일시' AFTER TRANSFERRED_FROM;

-- TB_ITEM 이관 인덱스
CREATE INDEX IDX_ITEM_TRANSFERRED ON TB_ITEM (TRANSFERRED_FROM);

-- ============================================
-- TB_BOARD_SHARE PERMISSION 컬럼 수정
-- MEMBER/VIEWER → VIEW/EDIT/FULL
-- ============================================
ALTER TABLE TB_BOARD_SHARE
    MODIFY COLUMN PERMISSION VARCHAR(20) NOT NULL DEFAULT 'VIEW' COMMENT '권한 (VIEW/EDIT/FULL)';

-- 기존 데이터 마이그레이션 (초기 설치 시에는 데이터가 없으므로 영향 없음)
